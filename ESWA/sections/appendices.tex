...
\texttt{5CD-AI/Vietnamese-Sentiment-visobert}; an unavailable or failed route
falls back to the deterministic lexicon implemented in the same module. The
supplied cache schema stores the resulting label, numeric score, and confidence,
but not the actual scorer, model identifier, or fallback flag. The audit therefore
does not infer provenance from those numerical fields. Every such record is
classified as \texttt{legacy-unknown}; a record contributes to the verified
ViSoBERT or verified lexicon count only when it contains explicit scorer metadata.
This conservative rule prevents an endpoint configuration from being reported as
evidence that the endpoint successfully scored a particular article.

\begin{table}[htbp]
\centering
\caption{Ticker-selection strata and point-in-time sentiment coverage.}
\label{tab:ticker_sentiment_coverage}
\scriptsize
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{|l|>{\raggedright\arraybackslash}p{2.1cm}|X|r|r|r|r|}
\hline
\textbf{Ticker} & \textbf{Sector} & \textbf{Design stratum} &
\textbf{Dated/total} & \textbf{V/L/U} & \textbf{Mean 90d} & \textbf{$\geq 8$ points} \\
\hline
BHN & Food/beverages & State-associated, specialised & 20/20 & 0/0/20 & 1.15 & 0/20 \\
\hline
CMG & Information technology & Mid-cap & 20/20 & 0/0/20 & 0.85 & 0/20 \\
\hline
FPT & Information technology & VN30/large-cap & 20/20 & 0/0/20 & 1.45 & 2/20 \\
\hline
HVN & Aviation and tourism & State-associated & 20/20 & 0/0/20 & 9.45 & 19/20 \\
\hline
MBB & Banking & VN30/large-cap & 20/20 & 0/0/20 & 7.35 & 7/20 \\
\hline
MWG & Retail & VN30/large-cap & 20/20 & 0/0/20 & 6.05 & 8/20 \\
\hline
VCB & Banking & VN30/large-cap & 20/20 & 0/0/20 & 9.90 & 18/20 \\
\hline
VJC & Aviation and tourism & Large private carrier & 20/20 & 0/0/20 & 10.10 & 17/20 \\
\hline
VNM & Consumer food & VN30/large-cap & 20/20 & 0/0/20 & 5.60 & 2/20 \\
\hline
\textbf{All} & \multicolumn{2}{l|}{\textbf{Nine-symbol purposive sample}} &
\textbf{180/180} & \textbf{0/0/180} & \textbf{5.77} & \textbf{73/180} \\
\hline
\end{tabularx}
\begin{flushleft}
\footnotesize \textit{Notes:} V/L/U denotes verified ViSoBERT, verified lexicon,
and unknown scorer counts. ``Mean 90d'' is the mean number of dated articles in
the inclusive 90-day lookback at the 20 test cutoffs. The final column counts
test points with at least eight eligible articles. The underlying machine-readable
audit is generated by \texttt{scripts/analyze\_sentiment\_coverage.py}.
\end{flushleft}
\end{table}
...
\item \textbf{Historical sentiment cache}: Nine JSON files (\texttt{sentiment\_cache\_\{SYMBOL\}.json}), one per evaluated stock, contain pre-crawled and scored CafeF articles. Point-in-time filtering uses the parsed publication date; undated records are rejected in strict research mode. The supplied files lack scorer metadata, so all 180 cached scores are labelled \texttt{legacy-unknown} in the coverage audit.
