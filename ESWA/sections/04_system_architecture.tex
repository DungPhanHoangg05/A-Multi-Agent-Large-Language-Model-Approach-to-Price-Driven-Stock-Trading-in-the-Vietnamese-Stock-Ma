...
An offline registry of exactly 85 candidate alpha functions is maintained, comprising five proprietary single-stock momentum and reversion formulas (Table~\ref{tab:proprietary_alphas}) and 80 adaptations drawn from the WorldQuant 101 Formulaic Alphas. The remaining 21 source expressions are not registered, evaluated, or counted; hence 101 denotes the source catalogue, whereas 85 denotes the executable candidate set. The previously reported value 87 is not used anywhere in the implementation or experiments. Cross-sectional operators are replaced by single-asset time-series surrogates as formalised in Eq.~\eqref{eq:ts_rank_localised}. This translation is heuristic: without contemporaneous peer dispersion, a rolling percentile answers ``where is this stock relative to its own recent history?'' rather than ``where is this stock relative to other stocks now?'' The adaptation can therefore change factor scale, persistence, turnover, and economic meaning. The present experiments do not estimate per-factor IC-decay curves, lagged signal autocorrelation, or turnover profiles, and do not claim that the adapted factors inherit the properties of their WorldQuant sources. When the volume series is unavailable or sparse, intra-bar price range is used as a proxy, creating an additional interpretation change for volume-dependent expressions.
...
Up to five eligible candidates are selected for the current prediction. When historical IC is negative, the signal is multiplied by $-1$ for metric computation and current inference. All observations used for this orientation precede the decision cutoff, which prevents lookahead into the evaluated trade. It does not eliminate selection bias: the same trailing observations determine the IC sign, score all 85 candidates, and choose the five winners. A noisy negative IC can therefore be converted into an apparently useful positive orientation, and the maximum over many noisy scores is optimistic even when every timestamp is cutoff-safe. No nested inner-selection/outer-evaluation loop, Deflated Sharpe Ratio, or rolling inclusion/sign-stability diagnostic is applied in the reported study. Candidate IC and Sharpe values are consequently descriptive ranking statistics, not multiplicity-adjusted evidence that a particular factor is genuinely profitable. Nested validation that repeats orientation and top-five selection inside every training fold~\cite{varma2006bias}, together with Deflated Sharpe or false-discovery control~\cite{bailey2014deflated,harvey2016}, is required for stronger factor-level claims.
...
\paragraph{Scope of visual verification.}
The two deterministic checks are \emph{output-consistency gates}: they test whether the final directional statement contradicts a small set of OHLCV-derived facts. They do not reveal which image region caused the model's answer, verify every claimed candle or trendline coordinate, or demonstrate sensitivity to the purported local pattern. A model that simply extrapolates the global trend can pass both gates whenever the shortcut and the rules agree. This distinction matters because observational financial charts couple local evidence with trend, making ordinary accuracy unable to identify grounding; controlled evidence interventions are needed to separate a grounded responder from a trend-shortcut responder \cite{wang2026martingale}. Accordingly, the present paper claims reduced obvious contradiction, not audited visual evidence faithfulness. A direct audit would require matched charts that preserve global trend while altering only local candlestick semantics, complementary trend swaps that preserve the local pattern, and measurement of whether the model's direction changes in the rule-implied way. FinChart-Bench further cautions that current models remain unreliable on spatial and instruction-following aspects of financial charts \cite{shu2026finchart}.
