...
While the system incorporates the WorldQuant-101 corpus, these formulas were originally designed for cross-sectional ranking across a broad, highly liquid equity universe. To apply them to the Vietnamese market's single-stock setting, we introduce a \emph{semantic translation layer} that redefines cross-sectional operators into rolling time-series equivalents.

\paragraph{Operator Localisation.}
The primary transformation involves the cross-sectional $\operatorname{rank}(x)$ operator, which denotes the relative percentile of a stock against its peers. We adapt this to a time-series percentile rank $\operatorname{ts\_rank}(x_u, w)$, representing the position of the current value $x_u$ relative to its own history over a rolling window $w$:
\begin{equation}
    \operatorname{ts\_rank}(x_u, w) \;=\; \frac{1}{w-1} \sum_{i=1}^{w-1} \mathbf{1}\!\left[x_u > x_{u-i}\right].
\end{equation}
The window parameter $w$ is inherited from the original WQ-101 specification and is typically between 5 and 20 bars, depending on the formula. It is kept unchanged to preserve each formula's intended look-back horizon. Operators that reference cross-sectional averages or market returns are re-parametrised using the asset's own rolling history: Average Daily Volume ($\mathrm{ADV}_{n}$) uses a $n$-day rolling mean of the asset's volume ($n = 10$ or $20$ as specified in the original formula), and the Volume Weighted Average Price (VWAP) is approximated by the daily intra-bar proxy $(H_u + L_u + C_u)/3$. Time-series functions such as $\delta(x, d)$ and $\operatorname{delay}(x, d)$ are implemented as $d$-period first differences and lags respectively, with $d$ taken directly from the original formula. The minimum look-back required to compute a candidate is at least $\max(w) + d$ valid observations; candidates that cannot be evaluated within the 60-candle minimum history are excluded from selection at that test point.
...
\begin{equation}
    ...
    \;=\;
    \tanh\!\left(
        \frac{\alpha^{(k)}_u - \mu_k}{\sigma_k + \varepsilon}
    \right),
    \label{eq:alpha_norm}
\end{equation}
where $\mu_k$ and $\sigma_k$ are the rolling mean and standard deviation
of $\alpha^{(k)}$ over the calibration history, and $\varepsilon > 0$
