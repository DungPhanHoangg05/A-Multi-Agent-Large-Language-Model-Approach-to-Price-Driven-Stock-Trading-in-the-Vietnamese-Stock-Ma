...
- **Corrective Gate Status:** `FIXED` — 76/76 regression tests pass bằng Python 3.13. Decision Agent và parser tuân thủ output nhị phân tuyệt đối: output `None`/rỗng/NEUTRAL/malformed chuyển thành `SHORT` thận trọng có provenance `fallback_conservative`; lỗi runtime của một decision variant không làm mất kết quả variant còn lại; lỗi upstream làm dừng benchmark thay vì ghi điểm `UNKNOWN`. Alpha selection, nhãn accuracy và account P&L cùng dùng mục tiêu net Open-to-Close; Full/No-Alpha có danh mục BUY/HOLD/SELL độc lập với vốn đầu kỳ 50 triệu VND và phí từng chiều; TASK-08 benchmark vẫn chưa chạy lại.
- **Priority:** `P0` / `P1`
...
- **Acceptance Criteria:**
  ...
  - Mọi điểm hoàn thành phải có quyết định nhị phân `LONG` hoặc `SHORT`; không lưu `UNKNOWN`. Nếu Decision Agent không trả phán quyết hợp lệ sau retry, dùng `SHORT` thận trọng và ghi rõ `decision_source=fallback_conservative`; lỗi upstream phải dừng run.
