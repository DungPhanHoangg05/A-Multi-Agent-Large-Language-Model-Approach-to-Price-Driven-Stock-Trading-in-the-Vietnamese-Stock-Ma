...
> **Mô hình thực thi:** **Single-Agent Tuyến tính (Strictly Sequential Execution by 1 Coding Agent)**.  
> **Source of Truth duy nhất** cho quá trình kiểm thử, sửa đổi kiến trúc, chạy thực nghiệm và đồng bộ bài báo.  
> **Quy chuẩn phát triển:** Bắt buộc tuân thủ [`AGENTS.md`](file:///AGENTS.md) (tạo branch riêng cho mỗi task, merge vào `develop` sau khi pass 100% tests, không over-engineering, không hallucination, dùng Python 3.13).  
...
| **ISSUE-07** | **Thiếu siêu dữ liệu xuất xứ mô hình Sentiment (Provenance Tracking)** | `PARTIAL` | `agents/sentiment_agent.py`: hàm `_predict` rơi về lexicon khi lỗi HF API nhưng cache cũ không ghi nhận `scorer`/`is_fallback`. `scripts/analyze_sentiment_coverage.py` không suy diễn từ score và phân loại 180/180 bản ghi là `legacy-unknown`. | Paper (§5.1, Appendix): công bố rõ verified ViSoBERT = 0/180, verified lexicon = 0/180 và unknown = 180/180; không còn tuyên bố provenance không thể kiểm chứng. | **P1** | `TASK-03`, `TASK-12` |
...
## 4. Critical Path (Chuỗi Thực thi Tuyến tính cho 1 Agent)

Vì toàn bộ công việc do **1 Coding Agent** đảm nhiệm, Critical Path là chuỗi phụ thuộc tuần tự tuyến tính (linear pipeline). Agent bắt buộc hoàn thành task trước, vượt qua Merge Gate vào nhánh `develop`, rồi mới chuyển sang task tiếp theo:
...
graph TD
    ...
    T01[Task 01: Fix Data Leakage in AlphaSelector]:::p0 -->|merge to dev| T02[Task 02: Implement Paired Shared Reports Protocol]:::p0
    T02 -->|merge to dev| T03[Task 03: Purge Undated Articles in SentimentCache]:::p0
    T03 -->|merge to dev| T04[Task 04: Implement Compounded Account PnL & Cost Model]:::p0
    T04 -->|merge to dev| T05[Task 05: Build Integrated Statistical Significance Suite]:::p0
    T05 -->|merge to dev| T06[Task 06: Decouple Alpha vs Sentiment in Ablation Engine]:::p1
    T06 -->|merge to dev| T07[Task 07: Add Rule-Based Verification to Vision Agents]:::p1
    T07 -->|merge to dev| T08[Task 08: Rerun 9-Symbol Benchmark & Generate Table 7]:::p1
    T08 -->|merge to dev| T09[Task 09: Rerun Robustness Sweep & Fix Table 8 Inconsistency]:::p1
    T09 -->|merge to dev| T10[Task 10: Run Disentangled Ablation Matrix]:::p1
...
- **Cổng 1 (Hoàn thành P0 Core - sau Task 03):** Toàn bộ đường ống nạp dữ liệu (Alpha & Sentiment) hoàn toàn sạch bóng data leakage; upstream snapshot được cố định cho cả 2 nhánh.
- **Cổng 2 (Hoàn thành P0 Math - sau Task 05):** P&L lãi kép, trừ phí giao dịch và kiểm định ý nghĩa thống kê (McNemar, Wilcoxon, Bootstrap) đã tích hợp sâu vào engine.
...
- **File/Function cần sửa:**
  ...
  - `agents/agent_state.py`: `IndicatorAgentState`
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-01-fix-alpha-leakage`.
  2. Cập nhật chữ ký hàm `select_top_alphas` trong `utils/alpha_selector.py` để nhận `historical_df` và `as_of_date`.
  3. Cập nhật `agents/agent_state.py` bổ sung trường `point_in_time_df` hoặc `as_of_date` vào `IndicatorAgentState`.
  4. Cập nhật `agents/alpha_agent.py` lấy `historical_df` từ state và truyền vào `select_top_alphas`.
  5. Cập nhật `core/backtest_engine.py` tại `_run_single()`: đưa `df.iloc[:end_idx]` vào initial state.
  6. Viết script kiểm thử `tests/test_alpha_leakage.py` xác minh `max(df.index) <= as_of_date`.
  7. Chạy test, xác nhận PASS, commit và merge vào `develop`.
- **Dependencies:** Không.
- **Test/Command cần chạy:**
  `py -3.13 tests/test_alpha_leakage.py`
...
- **Vấn đề hiện tại:** `core/backtest_engine.py` đang thực thi `_run_single(self._graph_full, ...)` sau đó tiếp tục thực thi `_run_single(self._graph_no_alpha, ...)`. Điều này khiến LLM phân tích lại mô hình nến, đường xu hướng và chỉ báo kỹ thuật cho nhánh No-Alpha. Do tính ngẫu nhiên của LLM, nhánh No-Alpha nhận báo cáo khác nhau ở mỗi lần chạy, dẫn đến hiện tượng vô lý trong Table 8: độ chính xác No-Alpha nhảy từ 28.6% lên 71.4% khi chỉ thay đổi tham số alpha.
- **Evidence:**
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-02-paired-shared-reports`.
  2. Sửa `utils/graph_setup.py`: Thêm phương thức biên dịch subgraph `compile_upstream()` và `compile_decision(include_alpha=True/False)`.
  3. Tái cấu trúc vòng lặp trong `core/backtest_engine.py` sang hàm `_run_paired_point()`.
  4. Viết script test `tests/test_paired_protocol.py` kiểm tra: chạy 3 lần với 3 cấu hình alpha khác nhau, kết quả của No-Alpha trên từng test point phải giống nhau từng chữ.
  5. Chạy test, xác nhận PASS, commit và merge vào `develop`.
- **Dependencies:** `TASK-01`.
- **Test/Command cần chạy:**
  `py -3.13 tests/test_paired_protocol.py`
- **Expected Output:** Vector `[tp.pred_no_alpha for tp in test_points]` đồng nhất tuyệt đối trên mọi cấu hình tham số alpha.
- **Acceptance Criteria:**
...
### [TASK-03] [Thứ tự: #3] [P0] Loại bỏ Rò rỉ Bài báo Không ngày trong Sentiment Cache

- **Task ID:** `TASK-03`
- **Status:** `COMPLETED` — kiểm thử loại bài không ngày/tương lai và neutral fallback PASS bằng Python 3.13.
- **Git Branch:** `task/TASK-03-purge-undated-sentiment`
- **Priority:** `P0`
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-03-purge-undated-sentiment`.
  2. Mở `data_manager/sentiment_cache.py`. Sửa hàm `get_at`: loại bỏ logic cộng thêm `no_date_articles`.
  ...
  5. Viết unit test trong `tests/test_sentiment_leakage.py`. Chạy test, xác nhận PASS, commit và merge vào `develop`.
- **Dependencies:** `TASK-02`.
- **Test/Command cần chạy:**
  `py -3.13 tests/test_sentiment_leakage.py`
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-04-compounded-account-pnl`.
...
- **Test/Command cần chạy:**
  `py -3.13 tests/test_account_pnl.py`
...
### [TASK-05] [Thứ tự: #5] [P0] Xây dựng Bộ Kiểm định Ý nghĩa Thống kê Tích hợp (Statistical Significance Suite)

- **Task ID:** `TASK-05`
- **Status:** `COMPLETED` — 8 kiểm thử McNemar exact, Wilcoxon 9 mã, Newey-West, Block Bootstrap CI và JSON summary PASS bằng Python 3.13; script tổng hợp chạy thành công trên đủ 9 kết quả hiện có.
- **Git Branch:** `task/TASK-05-integrated-significance-suite`
- **Priority:** `P0`
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-05-integrated-significance-suite`.
  2. Cập nhật `utils/statistical_tests.py` đảm bảo xử lý an toàn khi số lượng quan sát nhỏ (sử dụng exact binomial test cho McNemar khi $b+c < 25$).
  3. Bổ sung hàm `paired_wilcoxon_test(lifts_per_symbol: List[float]) -> Dict`.
  4. Tích hợp lệnh gọi thống kê vào `core/backtest_engine.py`.
  5. Viết script `tests/test_significance.py` xác minh tính toán trên dữ liệu giả lập có kiểm chứng.
  6. Chạy test, xác nhận PASS, commit và merge vào `develop`.
...
### [TASK-06] [Thứ tự: #6] [P1] Module hóa Tách rời Alpha định lượng và Sentiment tin tức (4-Way Ablation)

- **Task ID:** `TASK-06`
- **Status:** `COMPLETED` — 8 kiểm thử bốn cấu hình graph, prompt Việt/Anh, cô lập Alpha/Sentiment và shared upstream PASS bằng Python 3.13; toàn bộ 32 regression tests PASS.
- **Git Branch:** `task/TASK-06-decouple-alpha-sentiment`
- **Priority:** `P1`
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-06-decouple-alpha-sentiment`.
  2. Chỉnh sửa `utils/graph_setup.py` để hỗ trợ 4 chế độ đồ thị trên.
  3. Cập nhật `agents/decision_agent.py`: điều chỉnh bộ tạo prompt để xử lý độc lập mục `[4] ALPHA FACTORS` và `[5] SENTIMENT`.
  4. Cập nhật `core/backtest_engine.py` thêm tham số `variant` hoặc chạy đa biến thể.
  5. Viết test `tests/test_4way_ablation.py` xác nhận 4 biến thể biên dịch và chạy độc lập.
  6. Chạy test, xác nhận PASS, commit và merge vào `develop`.
- **Dependencies:** `TASK-05`.
- **Test/Command cần chạy:**
  `py -3.13 tests/test_4way_ablation.py`
...
### [TASK-07] [Thứ tự: #7] [P1] Bổ sung Cơ chế Kiểm tra Đối chiếu Luật cho Tác nhân Thị giác (LVLM Verification)

- **Task ID:** `TASK-07`
- **Status:** `COMPLETED` — 10 kiểm thử đối chiếu Pattern/Trend song ngữ, hạ Confidence, cảnh báo downstream, dữ liệu thiếu và tránh false positive PASS bằng Python 3.13; toàn bộ 42 regression tests PASS.
- **Git Branch:** `task/TASK-07-rule-based-vision-verification`
- **Priority:** `P1`
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-07-rule-based-vision-verification`.
  2. Viết hàm kiểm định quy tắc nến đơn giản trong `agents/pattern_agent.py`.
  3. Viết hàm kiểm định độ dốc xu hướng trong `agents/trend_agent.py`.
  4. Tích hợp vào hàm xử lý báo cáo trước khi trả về state.
  5. Viết unit test trong `tests/test_vision_verification.py`. Chạy test, xác nhận PASS, commit và merge vào `develop`.
- **Dependencies:** `TASK-06`.
- **Test/Command cần chạy:**
  `py -3.13 tests/test_vision_verification.py`
...
### [TASK-08] [Thứ tự: #8] [P1] Chạy lại Toàn bộ Benchmark 9 Mã Cổ phiếu và Xuất Bảng 7 Mới

- **Task ID:** `TASK-08`
- **Status:** `COMPLETED` — đã nghiệm thu 9 mã × 20 điểm kiểm tra (180/180), không có quyết định `UNKNOWN` hoặc bản ghi lỗi; đầy đủ McNemar, Block Bootstrap CI, Newey–West, P&L tài khoản và Wilcoxon tổng thể (`p=0.007812`) bằng Python 3.13.
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-08-rerun-9symbol-benchmark`.
  2. Kiểm tra API key Groq sẵn sàng trong `.env`.
  3. Khởi chạy script chạy hàng loạt 9 mã (có cơ chế checkpoint lưu từng test point để chống gián đoạn mạng).
  4. Kiểm tra tính hợp lệ của 9 file JSON xuất ra.
  5. Chạy script `utils/aggregate_benchmarks.py` để in ra bảng số liệu định dạng LaTeX Table 7 mới.
...
- **Acceptance Criteria:**
  ...
  - Mọi điểm hoàn thành phải có quyết định nhị phân `LONG` hoặc `SHORT`; không lưu `UNKNOWN` và không tự gán hướng fallback. Decision Agent phải dùng Structured Output, retry khi sai schema; nếu vẫn không trả phán quyết hợp lệ hoặc một nhánh paired lỗi thì dừng run/checkpoint để không làm thiên lệch thống kê.
...
- **Estimated Complexity:** `L` (Tiến trình chạy mạng và gọi API).
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-09-rerun-robustness-sweep`.
  2. Cập nhật `core/run_robustness.py` tích hợp logic shared upstream snapshot.
  3. Đảm bảo $W$ được truyền đúng vào cả bộ tạo ảnh `generate_kline_image` và `generate_trend_image`.
  4. Chạy sweep Panel A (Window Size), Panel B (Normalization), Panel C (Weights) với $N=20$.
  5. Xác nhận cột `Acc No-α` trong Panel B và Panel C giữ nguyên giá trị không đổi.
  6. Commit và merge vào `develop`.
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-10-run-disentangled-ablation`.
  2. Viết script `scripts/run_ablation_matrix.py`.
  3. Thực thi trên FPT, VNM, VCB.
  4. Thu thập bảng kết quả tổng hợp JSON/CSV.
  5. Commit và merge vào `develop`.
...
- **Paper Impact:** Tạo mới Table 9 (Ablation Analysis) trong Section 6.2 của bài báo LaTeX.
- **Estimated Complexity:** `M`
...
- **Cách triển khai đề xuất:**
  ...
  2. Trong `06_backtest_results.tex`:
     ...
     - Bổ sung Table 9 phân tích Ablation bóc tách Alpha vs Sentiment từ `TASK-10`.
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-11-sync-latex-manuscript`.
  2. Đọc số liệu từ các file kết quả sạch của `TASK-08`, `TASK-09`, `TASK-10`.
  3. Chỉnh sửa từng file `.tex` tương ứng.
  4. Biên dịch kiểm tra bằng pdflatex để đảm bảo không lỗi cú pháp.
  5. Commit và merge vào `develop`.
...
### [TASK-12] [Thứ tự: #12] [P2] Bổ sung Tiêu chí Tuyển chọn 9 Mã Cổ phiếu và Bảng Thống kê Phủ Sentiment

- **Task ID:** `TASK-12`
- **Status:** `COMPLETED` — audit 9 mã/180 test point PASS; 180/180 bài có ngày, 73/180 test point có ít nhất 8 bài trong cửa sổ 90 ngày, và 180/180 scorer được báo cáo trung thực là `legacy-unknown` do cache không lưu provenance.
- **Git Branch:** `task/TASK-12-ticker-rationale-coverage`
- **Priority:** `P1` / `P2`
...
- **File/Function cần sửa:**
  - Tạo script phân tích thống kê `scripts/analyze_sentiment_coverage.py`.
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-12-ticker-rationale-coverage`.
  2. Chạy script phân tích coverage trên 9 mã cổ phiếu.
  3. Soạn thảo bảng LaTeX tổng hợp thông tin ngành, vốn hóa, số lượng bài báo và tỷ lệ ViSoBERT.
  4. Chèn vào `ESWA/sections/05_experimental_setup.tex` và `ESWA/sections/appendices.tex`.
  5. Commit và merge vào `develop`.
...
### [TASK-13] [Thứ tự: #13] [P2] Kiểm thử Toàn diện Đầu Cuối (Regression Test) & Đóng gói Môi trường

- **Task ID:** `TASK-13`
- **Git Branch:** `task/TASK-13-regression-test-packaging`
- **Priority:** `P2`
- **Objective:** Tạo script kiểm thử hồi quy tự động kiểm tra toàn bộ luồng pipeline từ nạp dữ liệu, tính alpha, crawling tin tức, tạo ảnh, suy luận đa tác nhân, đến tính lãi kép tài khoản và kiểm định thống kê; chuẩn hóa script cài đặt môi trường ảo Python 3.13; xác nhận Definition of Done.
...
- **Các bước thực hiện theo thứ tự:**
  1. `git checkout develop && git checkout -b task/TASK-13-regression-test-packaging`.
  2. Viết script kiểm thử đầu cuối.
  3. Chạy thử nghiệm thành công trên máy tính người dùng.
  4. Cập nhật tài liệu hướng dẫn tái lập thực nghiệm trong `README.md`.
  5. Commit và merge vào `develop`.
...
### 6.1. Quy trình Thực thi Chuẩn từng Task (Per-Task Execution Protocol)
1. **Khởi tạo:**
   - Đảm bảo đang ở nhánh `develop`: `git checkout develop`.
   - Kiểm tra working directory sạch sẽ (`git status`).
   - Tạo branch riêng cho task: `git checkout -b task/<TASK-ID>-<slug>`.
2. **Khảo sát & Tối thiểu hóa:**
   - Mở chính xác các file được liệt kê trong task.
   - Không sửa lan man sang các hàm ngoài phạm vi task.
3. **Kiểm chứng cục bộ (Test First):**
   - Chạy lệnh test quy định bằng Python 3.13: `py -3.13 <lệnh_test>`.
   - Nếu test fail, đọc traceback và sửa tiếp đến khi PASS 100%.
4. **Merge Gate & Commit:**
   - Commit code với message chuẩn: `git commit -m "fix/feat: <TASK-ID> <mô tả ngắn>"`.
   - Chuyển về `develop` và merge với cờ `--no-ff`:
     ```bash
     ...
     ```
5. **Cập nhật Tiến độ:**
   - Ghi nhận trạng thái hoàn thành vào `plan/FINAL_COMPLETION_PLAN.md` trước khi sang task tiếp theo.

### 6.2. Bản đồ Phân kỳ Thực hiện (Phased Roadmap)

| Giai đoạn | Tasks phụ trách | Mục tiêu cụ thể | Dự kiến thời gian |
| :--- | :--- | :--- | :---: |
| **Pha 1: Data & Integrity** | `TASK-01`, `TASK-02`, `TASK-03` | Vá toàn bộ rò rỉ dữ liệu (alpha, sentiment); cố định snapshot báo cáo thượng nguồn. | Ngày 1 |
| **Pha 2: Financial Math & Rigor** | `TASK-04`, `TASK-05` | Triển khai mô hình lãi kép tài khoản, trừ phí thực tế; tích hợp bộ kiểm định McNemar, Wilcoxon, Bootstrap. | Ngày 1–2 |
| **Pha 3: Decoupling & Reliability**| `TASK-06`, `TASK-07` | Tách rời độc lập Alpha vs Sentiment (4-way ablation); bổ sung quy tắc đối chiếu luật chống ảo giác thị giác. | Ngày 2 |
...
| :--- | :--- | :--- | :---: |
| **`sections/05_experimental_setup.tex`**<br>Dòng 4–6 | Đoạn `\paragraph{Research-status disclosure}` thừa nhận thực nghiệm chưa hoàn thiện trong runtime. | **Xóa bỏ hoàn toàn.** Khẳng định runtime đã hiện thực hóa 100% giao thức `paired_shared_reports` và hợp đồng tài khoản. | `TASK-02`, `TASK-04`, `TASK-11` |
| **`sections/05_experimental_setup.tex`**<br>Subsection 5.1 | Đoạn văn ngắn liệt kê tên 9 mã cổ phiếu mà không có tiêu chí. | Bổ sung bảng giải trình tiêu chí chọn 9 mã (đa dạng ngành, vốn hóa, thanh khoản) và bảng tỷ lệ phủ sentiment/ViSoBERT. | `TASK-12` |
| **`sections/05_experimental_setup.tex`**<br>Table 6 & Subsection 5.4 | Mô tả chi phí và phân biệt dự báo nhị phân với thực thi tiền mặt. | Cập nhật công thức tính lợi nhuận gộp lãi kép tài khoản và trích dẫn mã nguồn thực thi chính thức. | `TASK-04`, `TASK-11` |
| **`sections/06_backtest_results.tex`**<br>Table 7 (Main Results) | Số liệu cũ với chú thích "Legacy Sum", thiếu p-value và confidence intervals. | **Thay thế toàn bộ bảng:** Cập nhật số liệu mới, bổ sung cột McNemar p-value, 95% Bootstrap CI cho Alpha Lift, và PnL lãi kép. | `TASK-05`, `TASK-08`, `TASK-11` |
| **`sections/06_backtest_results.tex`**<br>Subsection 6.2 | Hiện tại chỉ có phần phân tích sơ bộ FPT và HVN dựa trên số liệu cũ. | Bổ sung **Table 9: Disentangled Ablation Study** (bóc tách 4 điều kiện Alpha vs Sentiment), chứng minh vai trò độc lập của từng module. | `TASK-10`, `TASK-11` |
| **`sections/06_backtest_results.tex`**<br>Table 8 (Robustness Sweep) | Cột `Acc No-α` nhảy loạn từ 28.6% đến 71.4%; chú thích thừa nhận "control instability issue". | **Thay thế toàn bộ bảng:** Cột `Acc No-α` bất biến tuyệt đối ở Panel B và C; số liệu Panel A được render đúng cửa sổ $W$; cỡ mẫu $N=20$. | `TASK-02`, `TASK-09`, `TASK-11` |
| **`sections/07_limitations.tex`**<br>Mục L1 & L2 | Các đoạn văn thừa nhận hạn chế về "chưa chạy A20 paired protocol" và "chưa chạy bootstrap CI". | Chuyển đổi thành phân tích sâu về các rủi ro thị trường thực tế (slippage, market impact), khẳng định các vấn đề kỹ thuật đã được kiểm soát. | `TASK-11` |
| **`sections/04_system_architecture.tex`**<br>Subsection 4.4 | Mô tả tác nhân thị giác chưa có cơ chế phòng chống ảo giác. | Bổ sung mô tả ngắn về cơ chế kiểm tra đối chiếu quy tắc nến/xu hướng (rule-based consistency check) chống ảo giác CHARTHAL. | `TASK-07` |
| **`sections/appendices.tex`**<br>Appendix B & C | Danh mục alpha và mẫu prompt. | Xác nhận danh mục đúng 85 alpha; bổ sung thống kê chi tiết coverage CafeF và log thực nghiệm mẫu. | `TASK-11`, `TASK-12` |
...
---
*Kế hoạch này là tài liệu chuẩn mực và kim chỉ nam duy nhất cho Agent thực thi.*
