...
def generate_backtest_summary_chart(summary: dict, output_path: str) -> None:
    ...
    try:
        ...
        ax1.set_title(f"Mô phỏng Lợi nhuận gộp lũy kế - {summary.get('symbol')}", fontsize=12, fontweight='bold', pad=10)
        ...
        ax2.set_title("Số lần dự đoán đúng lũy tiến", fontsize=11, pad=8)
        ax2.set_xlabel("Số thứ tự Test Point", fontsize=10)
        ax2.set_ylabel("Số lần đúng", fontsize=10)
        ...
        plt.figtext(0.5, 0.02, 
                    ...
                    f"Alpha Lift: {summary.get('alpha_lift'):+.1f}% | "
                    f"P&L Lift: {summary.get('pnl_lift'):+.1f}%", 
                    ha="center", fontsize=8, color="gray", style='italic')

        fig.savefig(output_path, dpi=150, bbox_inches="tight")
        plt.close(fig)
    except Exception as e:
        print(f"[static_util] Lỗi vẽ chart tổng kết: {e}")