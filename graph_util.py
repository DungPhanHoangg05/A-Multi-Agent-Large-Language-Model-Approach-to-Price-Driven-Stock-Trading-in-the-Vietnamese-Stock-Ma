...
def _to_clean_list(series, n: int = 28):
    """Giữ None thay vì fillna(0); loại bỏ NaN."""
...
class TechnicalTools:
    ...
    def generate_kline_image(
        ...
        """
        df = pd.DataFrame(kline_data).tail(40)
    ...
    def compute_macd(
        ...
        """Compute the MACD using TA-Lib."""
        df = pd.DataFrame(kline_data)
        macd, macd_signal, macd_hist = talib.MACD(
            df["Close"], fastperiod=fastperiod, slowperiod=slowperiod, signalperiod=signalperiod
        )
        return {
            "macd":        _to_clean_list(macd),
            "macd_signal": _to_clean_list(macd_signal),
            "macd_hist":   _to_clean_list(macd_hist),
        }
    ...
    def compute_stoch(
        ...
        return {"stoch_k": _to_clean_list(stoch_k), "stoch_d": _to_clean_list(stoch_d)}

    @staticmethod
    @tool
    def compute_roc(
        kline_data: Annotated[dict, "Dictionary with a 'Close' key."],
        period: Annotated[int, "Number of periods for ROC (default is 10)"] = 10,
    ) -> dict:
        """Compute the Rate of Change (ROC) indicator using TA-Lib."""
        df = pd.DataFrame(kline_data)
        roc = talib.ROC(df["Close"], timeperiod=period)
        return {"roc": _to_clean_list(roc)}
    ...
    def compute_willr(
        ...
        return {"willr": _to_clean_list(willr)}
