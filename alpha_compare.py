...
def normalize_alpha(series: pd.Series) -> pd.Series:
    """Clip to [-1, 1] via tanh of zscore — makes all alphas comparable."""
...
def run_backtest(df: pd.DataFrame, lookahead: int = 3,
    ...
    for i, (name, (fn, desc)) in enumerate(ALPHA_REGISTRY.items()):
        ...
        try:
            alpha_series = fn(d)
            alpha_norm   = normalize_alpha(alpha_series)
...
def rank_alphas(df_results: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    r["score_sharpe"] = norm01(r["sharpe"].clip(-3, 3))
